...
\end{figure}
To transform the long, confusing schedule from the example in Section \ref{ch:Introduction:sec:Motivation} into a periodic and clear schedule, the temporal graph needs to be broken down and transformed into an edge periodic graph.
...
Since we only represent unary automata with \textbf{$|\Sigma| = 1$} and therefore only have a single transition from each state, basically forming a simple circle of all possible states.
This means that for a period length of $i$ we only have to check $i$ states on the circle being in the same state.
Decomposing the obtained \DFA results in a family of smaller \DFAs, with $\cap^k_{i=1} L(\mathcal{A}_i)$ which can be interpreted as a set of binary labels $w_i$ and their bitwise intersection $\land^k_{i=1} w_i$ equals $w'$.
...
The decomposition of unary \DFAs is usually characterized by utilizing clean quotients. Let  $A = \lbrace\Sigma, Q, q_I , \sigma, F\rbrace$ be a unary \DFA.
