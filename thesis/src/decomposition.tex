...
Iterating over all states $q \in Q \backslash F$, the set $Q_{q,w_i} = \{\sigma(q,w^\lambda_i)|\lambda \geq |Q|\}$ is computing for each word $w_i \in \mathcal{W}$, which contains all the states reachable from $q$ when reading a word from $\mathcal{W}$.
If the intersection of $Q_{q,w_i}$ and the final states of $A$ is empty, $w_i$ covers all rejecting states, and if this is true for all words from the current word combination set, a valid decomposition is found.
