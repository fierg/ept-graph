...
\[ s_i \sim_d s_j ~\text{if and only if}~i=j \]
\[ q_i \sim_d q_j ~\text{if and only if}~i \equiv j ~mod~ d \]

Note that $\sim_d$ is coherent with $\sigma$, and so $L(A) \subseteq L(A_d)$. As with general quotient DFAs, the latter containment may be strict.
