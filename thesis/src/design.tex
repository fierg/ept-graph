...
Given a positive integer $n$, let $d(n)$ denote the number of divisors of $n$ (including 1 and $n$), thus for instance $d(6)=4$, and more generally, if $n$ has a prime factorization $n = p_1^{a_1} \ldots p_k^{a_k}$ then by the fundamental theorem of arithmetic $d(n) = (a_1+1) \ldots (a_k+1)$. Clearly, $d(n) \leq n$ and therefore $d(n) = n^{o(1)}$.  
