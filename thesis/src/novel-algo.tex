...
Usually the algorithms and proofs are considering unary \DFAs consisting of a chain leading into a cycle of states.
...
\begin{algorithm}[H]
	...
	\Fn{getFactor($A = \{{a}, Q, q_I , \sigma, F \}$: unary DFA, integer $d$)}{
		$F_d \gets \emptyset$
		$A_d \gets \{{a}, Q, q_I , \sigma, F_d \}$
		\ForEach{$i \in [1,d]$}{
			\If{isPeriodic($A$,$i$)}{
				$F_d \gets F_d \cup q_i$ // TODO
			}
		}
	}
	\Fn{isPeriodic($A = \{{a}, Q, q_I , \sigma, F \}$: unary DFA, integer $i$)}{
		\While{}{
			//TODO	
		}
	}
...
The original problem was solvable in \LogSpace, but extending the problem to finding the shortest possible factors, we face a different challenge.
...
In order to find lower bounds for this novel problem, we are going to reduce the {\sc{Set Cover}} problem, known to be NP-complete, to our novel {\sc{Min Factors Decomposition}} problem, using a polynomial time reduction.
...
If we allow for factors of original size $|B_i| \leq |A|$, we are able to find a factor of original size, which covers all values not covered by a factor so far.
...
Obviously this is not properly defined for \DFAs but it makes sense in the context of explainable labels.
