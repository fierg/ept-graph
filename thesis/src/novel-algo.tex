...
\begin{definition}[\sc{Set Cover}]{\ \\}
	Input: A set of elements $U = \{u_1, u_2, \dots , u_n\}$. A set $\mathcal{C} = \{C_1, C_2, \dots , C_m\}$ where $\forall~ i ~C_i \subseteq U$.\\
	Question: Exists a set $\mathcal{O} \subseteq \mathcal{C}$ of minimum size such that $\bigcup_{C_i \in \mathcal{O}} C_i = U$
\end{definition}
...
If the Set Cover problem finds a cover $\mathcal{O} \subseteq \mathcal{C}, |\mathcal{O}|=k$ with $\bigcup_{C_i \in O} C_i = U$, it means there is a set of indices $i_1, i_2, \dots, i_k$ that chooses from the words $w_i$ such that $w_{i_1}, w_{i_2}, \dots, w_{i_k}$ can form the same binary word as $u$, which satisfies the original problem. 

This reduction demonstrates that solving the original problem is as hard as solving the Set Cover problem.
