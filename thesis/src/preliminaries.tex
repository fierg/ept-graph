...
When analyzing a decomposition family $(B_i)_{1 \leq i \leq k}$, a partial decomposition up to $j < k$ might have words which are accepted by $A$ but not by $(B_i)_{1 \leq i \leq j}$, those are called \enquote{outliers} with $\text{out}_{\{A_1,A_2, \dots, A_j\}} = $ words not covered by the decomposition up to $j$. 

TODO: numbers for unary dfas
