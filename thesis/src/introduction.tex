...
TODO: use references from state of the art section
%% ==============================
...
To use the benefits of EPG's, a transformation from temporal to egde periodic has to be performed. This decomposition can be split up into multiple steps. Initially, we read the label and interpret it as a unary automata. Labels consisting of binary values can be interpreted as unary DFAs by representing either the 0 or 1 values as final states of the DFA, which representation will result in better EPG transformations will be a topic later on. 
