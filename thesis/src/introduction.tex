...
To use the benefits of the periodic representation as edge periodic graph, a transformation from temporal to edge periodic has to be performed. This transformation can be done in different ways, but we focus on a method based on the decomposition of deterministic finite automaton. Reading the labels of a temporal graph and interpreting it as \DFA enables the application of know methods for \DFA decomposition. Labels from a temporal graph, consisting of binary strings, can be interpreted as unary \DFAs by representing the 1 values as final states of a \DFA. 
\begin{figure}[h]
